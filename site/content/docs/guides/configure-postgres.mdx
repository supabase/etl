...
role does not have to own the tables. Run the schema and table grants in the
database being replicated. `ALTER DEFAULT PRIVILEGES` applies to future
objects created by the role executing it; repeat it with `FOR ROLE` for each
role that creates published tables.
...
sync workers create `supabase_etl_table_sync_{pipeline_id}_{table_id}` slots
for initial sync and drop them after catch-up completes. These are persistent
PostgreSQL slots with a short lifecycle, not session-scoped `TEMPORARY` slots;
they can remain after a failed or interrupted sync.
...
<Callout className="etl-callout" type="warn" title="Warning">
  PostgreSQL rejects attempts to drop an active slot. Dropping an inactive
  slot still discards its saved position and WAL-retention protection, so stop
  the pipeline and plan its recovery before deleting a slot it needs.
</Callout>
...
Each pipeline uses one apply-worker slot and additional slots for table sync
workers. Budget at least `max_table_sync_workers + 1` slots per pipeline for
normal operation, plus capacity for other replication clients and slots left
by interrupted or errored syncs. `max_table_sync_workers` limits concurrent
workers, not every slot that may remain in PostgreSQL.
...
Sets a minimum amount of recent WAL to retain for physical standbys. Logical
replication slots retain the WAL they need independently; `wal_keep_size` is
neither a slot-retention limit nor a guarantee against slot invalidation.
...
<Callout className="etl-callout" type="warn" title="Warning">
  `max_slot_wal_keep_size` limits slot-based WAL retention at checkpoint time.
  If required WAL is removed, the slot becomes unusable and replication needs
  recovery. This is not a hard cap on total WAL directory size:
</Callout>
...
- Alert on `confirmed_flush_lag_bytes`, `retained_wal_bytes`, `safe_wal_size`,
  and `wal_status`.
- Interpret `safe_wal_size` together with `wal_status` and
  `max_slot_wal_keep_size`: `NULL` can mean a lost slot or unlimited retention,
  while `0` means no remaining headroom. See
  [PostgreSQL's slot monitoring reference](https://www.postgresql.org/docs/18/view-pg-replication-slots.html).
