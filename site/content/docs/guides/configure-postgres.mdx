...
Enable the feature in the ETL pipeline configuration:

```yaml title="replicator.yaml"
pipeline:
  replication_slot:
    failover: true
```

For API-managed pipelines, set the same field in the pipeline configuration:
