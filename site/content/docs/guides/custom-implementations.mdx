...
Create `src/custom_store.rs`. A store must implement **three traits** (see [Extension Points](/explanation/traits/) for full details):
...
Startup loads all retained schemas, table states, destination metadata, and
worker checkpoints before destinations or workers run. Getters read the shared
cache; writes and lifecycle operations maintain it after persistence succeeds.
