...
A destination that stops on its own, for example with the process embedding
ETL, should request pipeline shutdown and then fail the work it abandons with
`ErrorKind::DestinationShutdown`, through the method return or the async
result. While the pipeline shuts down, ETL treats that failure like the
shutdown signal and records no table error, so the next start copies the
table again instead of waiting for a manual retry.
