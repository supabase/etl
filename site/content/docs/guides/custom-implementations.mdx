...
<Callout className="etl-callout" type="info" title="Note">
  ...
  20% batch-memory ratio is a global advisory target divided across potential
  concurrent batches; 32 MiB is the preferred ceiling for one batch, not a
  reservation. Emergency backpressure independently pauses new source polling
  at 85% usage and resumes below 75%, while already-owned destination work
  continues to drain. A single decoded row can exceed the target because rows
  are indivisible.
</Callout>
