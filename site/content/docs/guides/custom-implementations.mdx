...
|-------|--------|------|-----------|
| Initial sync | `write_table_rows()` | Startup | `TableCopyWrite` |
...
use etl::destination::{
    ...
    TableCopyWrite, WriteEventsResult, WriteTableRowsResult,
};
use etl::error::{ErrorKind, EtlResult};
use etl::{event::Event, schema::ReplicatedTableSchema};
...
impl Destination for HttpDestination {
    ...
    async fn write_table_rows(
        ...
        let (batch_id, rows) = batch.into_parts();
        let table_name = replicated_table_schema.name().to_string();
