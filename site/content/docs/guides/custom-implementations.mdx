...
`write_events()` batches. If your destination keeps physical schemas, flush
writes that still use the previous schema, apply the supported operations in
the supplied order, then continue with following rows. See
...
- `CachedStore` - Complete cache preparation before startup
