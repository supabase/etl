...
`shutdown()` is ETL stopping your destination. A destination that stops on its
own, for example together with the process embedding ETL, instead answers the
work it abandons by calling `shutdown()` on the async result rather than
`send()`. ETL then requests pipeline shutdown through its normal path and
records no table error, so the next start resumes the table from its persisted
state.
