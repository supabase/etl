...
<Callout className="etl-callout" type="info" title="Note">
  ...
  20% batch-memory ratio is a global advisory target divided across accumulating
  and in-flight batches; 32 MiB is the preferred ceiling for one batch, not a
  reservation. ETL can reduce that target as selected system or cgroup usage
  rises. Emergency backpressure pauses new source polling at 85% usage and
  resumes below 75%, while already-owned destination work continues to drain. A
  single decoded row can exceed the target because rows are indivisible.
</Callout>
