...
Implement a store and an HTTP destination. Start with
[First Pipeline](/guides/first-pipeline/) or be familiar with ETL basics.
Your implementation can live entirely in your own project; it does not need to
be accepted as an official built-in destination. See
[Destinations](/reference/destinations/#custom-destinations-and-upstream-support)
for the distinction and upstream maintenance expectations.

## Understanding the Destination Trait

ETL uses two write methods. Initial sync uses both: it copies existing rows,
then catches up changes before ongoing replication begins.

| Method | When | Data |
| --- | --- | --- |
| `write_table_rows()` | Copying existing rows during initial sync | `Option<TableCopyBatchId>` and `Vec<TableRow>` |
| `write_events()` | Catch-up and ongoing replication | `Vec<Event>` including `Relation` schema events |

<Callout className="etl-callout" type="info" title="Note">
  During initial sync, parallel table sync workers each process their own
  replication slot, so `Begin` and `Commit` transaction markers may appear
  multiple times. These repeated markers do not by themselves duplicate row
  events, but ETL is at-least-once, so destination writes should still be
  idempotent across retries and restarts.
</Callout>

Schema notifications arrive as ordered `Event::Relation` values inside
`write_events()` batches. Compare the snapshot and replication mask with applied
metadata first; repeated identical notifications need no transition. If your
destination keeps physical schemas, flush writes that still use the previous
schema before applying a supported transition, then continue with following
rows. See
[Schema Changes](/explanation/schema-changes/) for publication-mask additions,
defaults, recovery, and current limitations.

## Create the project [#create-the-project] [step]

```bash title="Terminal"
cargo new etl-custom
cd etl-custom
...
## Implement a custom store [#implement-a-custom-store] [step]

Create `src/custom_store.rs`. A store must implement **four traits** (see [Extension Points](/explanation/traits/) for full details):
...
The state, schema, and lifecycle traits inherit `CachedStore`, so helpers do
not repeat that bound when using one of those capabilities.

Startup calls `CachedStore::load_cache()` once to prepare all retained schemas,
table states, destination metadata, and worker checkpoints before initialization
reads, destinations, or workers run. Persistent implementations load these
together and publish a complete cache only after success. Failed or cancelled
loads must not expose partial state. Getters can recover an unusable cache;
writes and lifecycle operations maintain it after persistence succeeds.

For a persistent cache, an interrupted commit can leave the database outcome
unknown even when the Rust future has been dropped. Prevent subsequent access
from trusting stale data until recovery has established the committed state.
See [PostgreSQL cache concurrency and recovery](/explanation/traits/#postgresql-ownership-and-concurrency)
for the built-in store's ownership contract, locking protocol, isolation level,
and recovery behavior.

<Callout className="etl-callout" type="warn" title="Instructional store">
  This example keeps state in memory so the trait contract is visible in one
  file, including rollback during an in-process retry. It does not survive a
  process restart. A production store must durably and atomically persist state
  history, checkpoints, schemas, and destination metadata before acknowledging
  each write.
</Callout>

```rust title="src/custom_store.rs" lineNumbers
...
use etl::{
    ...
    store::{
        CachedStore, SchemaStore, StateStore, TableState, TableStateLifecycleStore, TableStateOperation,
...
impl CachedStore for CustomStore {
    async fn load_cache(&self) -> EtlResult<()> {
        // This store keeps its authoritative state in memory.
        Ok(())
    }
}
...
The module will be compiled when it is declared in `src/main.rs` in the
[Wire it together](#wire-it-together) step.

## Implement a custom destination [#implement-a-custom-destination] [step]
...
ETL is **at least once**, so make row and event writes idempotent. Complete
every async result handle: send `DestinationWriteStatus::Durable` from this
immediate example after a successful write. ETL clears copy-scoped store state
only after `drop_table_for_copy()` succeeds, so use the supplied schema to find
the object to remove. See [Extension Points](/explanation/traits/#destination)
for deferred `Accepted` writes, copy batch IDs, empty durability barriers, and
startup/shutdown hooks.

The example runs HTTP event writes in a `TaskRegistry` so `write_events()` can
return promptly and the apply loop can keep progressing. Each task owns only
its client, URL, payload, and result handle; it does not capture the registry.
The destination drains those tasks in `shutdown()` after workers complete
successfully. A worker failure skips this hook; dropping the destination's last
registry owner requests cancellation without waiting. A panic during draining
also returns immediately and drops remaining task handles. Copy writes remain
inline because ETL already runs copy partitions in separate workers. They can be
cancelled mid-request, so the remote `drop-for-copy` endpoint must ensure that
requests from an earlier copy cannot modify the replacement table.

```rust title="src/http_destination.rs" lineNumbers
use reqwest::Client;
use serde_json::json;
...
impl HttpDestination {
    pub fn new(base_url: String) -> EtlResult<Self> {
        ...
        Ok(Self { client, base_url, tasks: TaskRegistry::new() })
    }

    async fn post(client: &Client, url: &str, body: serde_json::Value) -> EtlResult<()> {
        for attempt in 1..=3 {
...
impl Destination for HttpDestination {
    ...
    async fn shutdown(&self) -> EtlResult<()> {
        let _guard = self.tasks.drain().await?;
        Ok(())
    }
    ...
    async fn drop_table_for_copy(
        ...
        let url = format!("{}/tables/{table_name}/drop-for-copy", self.base_url.trim_end_matches('/'));
        let result = Self::post(&self.client, &url, json!({})).await;
    ...
    async fn write_table_rows(
        ...
        let result = Self::post(&self.client, &url, payload)
            .await
    ...
    async fn write_events(
        ...
        if events.is_empty() {
            // This destination never returns Accepted, so it has no
        ...
        self.tasks.spawn(async move {
            let result = Self::post(&client, &url, payload).await;
...
The module will be compiled after the next step wires it into the pipeline.
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    // Keep the same completion future alive while requesting shutdown.
    let pipeline_wait = pipeline.wait();
    tokio::pin!(pipeline_wait);
    tokio::select! {
        result = &mut pipeline_wait => result?,
...
<Callout className="etl-callout" type="info" title="Note">
  Update the database name, password, and HTTP endpoint to match your setup. The
  20% batch-memory ratio is a global advisory target divided across positions
  that can simultaneously hold an accumulating or in-flight decoded batch. The
  32 MiB value is the preferred ceiling for one batch, not a reservation.
  Emergency backpressure independently pauses new source polling at 85% usage
  and resumes below 75%, while already-owned destination work continues to
  drain. A single decoded row can exceed the target because rows are indivisible.
</Callout>

## Test the pipeline [#test-the-pipeline] [step]

First, compile the complete example:

```bash title="Terminal"
cargo check
```

Before running, create `my_publication` in your database following
[Configure Postgres](/guides/configure-postgres/#publications), and replace the
placeholder endpoint with an HTTP service that implements the example's
`rows`, `events`, and `tables/{table_name}/drop-for-copy` routes.

This HTTP example demonstrates the destination contract, not a complete
replication format: row values use debug strings, and change events include
only their type and table name. A production implementation needs typed row
payloads, sequence keys, schema handling, and idempotent writes. The service
must acknowledge success only after the submitted work is durable, because
this adapter treats a successful HTTP response as `Durable`.

```bash title="Terminal"
cargo run
```

The pipeline logs state transitions and sends HTTP requests to your service.

## Next Steps

- [Extension Points](/explanation/traits/) - Full trait API documentation
- [Events](/explanation/events/) - Details on all events your destination receives
- [Configure Postgres](/guides/configure-postgres/) - Production database setup
