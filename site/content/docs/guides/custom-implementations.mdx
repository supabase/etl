...
|-------|--------|------|-----------|
| Initial sync | `write_table_rows()` | Startup | `Option<TableCopyBatchId>` and `Vec<TableRow>` |
| Ongoing replication | `write_events()` | During catch-up and ongoing replication | `Vec<Event>` including `Relation` schema events |
...
keeps physical schemas, flush writes using the previous schema, build a
destination-aware plan, validate the complete plan, and apply its operations in
the supplied order before processing following rows. Each alteration contains
adjacent before and after column schemas. A default addition, removal, or
replacement is one `Default` alteration; remove the old default idempotently,
then set the new one only when currently supported.

The `reason` carried by `SchemaOperation::AddColumn` distinguishes a physical
PostgreSQL table addition from a column newly exposed by a publication
replication mask. For a replication-mask addition, always create the destination
column as nullable: its historical values were never replicated, even when the
source column is `NOT NULL`. Keep those existing values `NULL`. Install source
default metadata only if the destination guarantees that setting the default
affects future writes without changing or virtually projecting values for
existing rows. If the destination type cannot be nullable, reject the schema
change before DDL and require resynchronization. Future ETL row events already
contain the value evaluated by PostgreSQL.

See [Schema Changes](/explanation/schema-changes/) for built-in destination
behavior, recovery rules, and current limitations.
