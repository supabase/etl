...
- PostgreSQL 14 through 18 configured for logical replication, including a
  ...
  [Configure Postgres](/guides/configure-postgres/) first.
- For this BigQuery walkthrough: a Google Cloud project with the BigQuery API
  enabled, a dataset for the replicated tables, and a service account with the
...
read replica. Other built-in destinations use the same binary with a different
`--features` flag; see [Destinations](/reference/destinations/).
