...
not durable replication progress. Slot acquisition and intentional catchup waits
are exempt. With no observations, the process stays live but unready. Apply loops
...
`replicator.health.startup`, `.liveness`, and `.readiness`. Each supplied block
