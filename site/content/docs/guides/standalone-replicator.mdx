...
Probes observe apply-loop iterations and table-copy work, not durable replication
progress. Slot acquisition and intentional catchup waits are exempt. With no
observations, the process stays live but unready. Apply loops allow the greater
of `stall_timeout_ms` and PostgreSQL's `wal_sender_timeout` (60-second fallback
when disabled or unavailable). Tune the stall timeout above expected copy batch
and stage durations.

For Kubernetes, use `/livez` for startup and liveness, and `/readyz` for readiness
on the configured port. The optional ETL API accepts this block under
`replicator.health` and generates the probes automatically; enable it only with
a replicator image that supports these endpoints.
