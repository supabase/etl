...
Use one store and its clones per pipeline to share cached state. External
changes to ETL state or destination data are unsupported.

Each store trait requires `CachedStore`, which loads all cache domains together
during startup, before workers run. See [Extension Points](/explanation/traits/#cachedstore)
for the cache and durability contracts.
...
ETL does not promise exactly-once delivery across its source, store, and
destinations. A failure can occur after the destination commits a write but
before ETL saves the corresponding checkpoint. Destinations can use idempotent
writes or deduplication to keep replay from changing the final result.
ETL also limits replay through:

- **Controlled shutdown**: The pipeline stops intake, drains apply work, and joins tasks. Interrupted initial copies restart from a fresh snapshot. Failures return immediately and request abort of remaining owned tasks; persisted checkpoints still govern recovery
...
| **SyncDone** | Table Sync Worker | Catch-up complete; awaiting a persisted apply checkpoint and local decoder before handover |
