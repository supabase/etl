---
title: Events
description: Understand the events Supabase ETL delivers during catch-up and ongoing replication.
icon: ListTree
---

During catch-up and ongoing replication, Supabase ETL delivers events decoded from Postgres
...
Provides **table schema and column-mask information** before subsequent data
events. A repeated notification does not necessarily mean the schema changed.
...
PostgreSQL relation messages are connection-local protocol metadata. A reconnect
or catalog invalidation, including maintenance, can resend an unchanged schema.
ETL can also synthesize a `RelationEvent` before an insert, update, or delete
when a pending DDL snapshot has no following protocol relation. These events
have no durable event sequence key and do not consume a transaction ordinal.

The carried `SnapshotId` identifies a stored table-schema version, while the
masks describe its replicated projection and row identity. Supported publication
column-list changes receive a new snapshot even when the full table schema is
unchanged. No-op DDL can also advance the snapshot without changing columns;
receiving a protocol relation alone does not create a new version.

Destinations compare the snapshot and replication mask with applied metadata.
An identical pair needs no schema transition. A newer snapshot must be
validated even when its projected schema needs no physical DDL. An equal
snapshot with a different replication mask is ambiguous and must be rejected.
Preserve event order; relation delivery is not a checkpoint or deduplication key.
...
**For checkpointing:** Let ETL advance its worker checkpoint after the
corresponding destination work is durable. A row's `commit_lsn` alone does not
prove that every event in that transaction has been delivered: batches can
split transactions. Custom stores must implement the
[replication checkpoint contract](/explanation/traits/#replication-checkpoint-methods);
destinations must report durability through the supplied result handles.
...
**Ordering requirement:** Preserve per-table event order, including schema
changes and truncates. Destinations may process independent work concurrently
only when they preserve those barriers and the required durability ordering.
