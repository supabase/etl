...
impl DuckLakePendingInlineSizeSampler {
    ...
    pub(super) async fn sample_table(
        ...
        Ok(record_pending_inline_data_sizes(inlined_data_bytes, table_name))
    }
}

/// Records sampled inline insert-data bytes for one table.
fn record_pending_inline_data_sizes(
    inlined_data_bytes: i64,
    table_name: &str,
) -> DuckLakePendingInlineDataSizes {
    let inlined_data_bytes = inlined_data_bytes.max(0) as u64;
    gauge!(ETL_DUCKLAKE_TABLE_ACTIVE_INLINED_DATA_BYTES, TABLE_LABEL => table_name.to_string())
        .set(inlined_data_bytes as f64);
    DuckLakePendingInlineDataSizes { inlined_data_bytes }
}
...
mod tests {
    ...
    async fn recording_pending_inline_data_sizes_exports_gauge_value() {
        ...
        assert_eq!(sizes, DuckLakePendingInlineDataSizes { inlined_data_bytes: 1_024 });
        assert!(
