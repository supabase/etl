...
impl TestApp {
    ...
    pub async fn set_default_image_by_name(&self, name: &str) -> reqwest::Response {
        self.post_authenticated(format!("{}/v1/images/default", &self.address))
            .json(&SetDefaultImageRequest {
                name: name.to_string(),
            })
            .send()
            .await
