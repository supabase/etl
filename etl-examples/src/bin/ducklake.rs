...
#[derive(Debug, Args)]
struct DuckLakeArgs {
    /// PostgreSQL connection string for the DuckLake catalog (e.g., postgres://user:pass@host/db)
    #[arg(long)]
    catalog_url: String,
    /// Local directory or cloud URI for Parquet files (e.g., ./lake_data/ or s3://bucket/)
    #[arg(long)]
    data_path: String,
