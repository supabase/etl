    ...
    D: Destination + Clone + Send + 'static,
{
    ...
    debug!("starting table sync for table {} in phase {:?}", table_id, phase_type);
    ...
    if matches!(
        ...
        info!("table {} sync not required, already in phase {:?}", table_id, phase_type);
    ...
    if !matches!(
        ...
        error!("invalid replication phase {:?} for table {}, cannot perform table sync", phase_type, table_id);
