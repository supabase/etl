...
pub(crate) fn register_metrics() {
    REGISTER_METRICS.call_once(|| {
        ...
        describe_counter!(
            ...
            Unit::Count,
            "Total number of failed heartbeat attempts"
        );

        describe_gauge!(
            ETL_HEARTBEAT_CONSECUTIVE_FAILURES,
            Unit::Count,
            "Current number of consecutive heartbeat failures (resets on success)"
        );

        describe_counter!(
            ETL_HEARTBEAT_CONNECTION_ATTEMPTS_TOTAL,
            Unit::Count,
            "Total number of connection attempts to the primary database for heartbeat"
        );
