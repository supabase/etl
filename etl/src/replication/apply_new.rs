//! New apply loop implementation without hook trait interface.
//!
//! This module provides a refactored apply loop that uses a `WorkerContext` enum
//! instead of the `ApplyLoopHook` trait, enabling more flexible worker-specific
//! behavior at different points in the replication processing cycle.
