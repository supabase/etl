...
//!
//! When replicating from a PostgreSQL read replica (PG 15+), the replication slot on the
//! primary may become inactive if no WAL activity occurs. This worker emits periodic
//! heartbeat messages to the primary database using `pg_logical_emit_message()` to generate
//! WAL activity and keep the slot active.
...
///
/// PostgreSQL 15 introduced the ability to replicate from standby servers,
/// which is when heartbeat functionality becomes necessary.
...
///
/// Used internally to monitor the worker's lifecycle and connection health.
...
///
/// This handle allows the caller to wait for the heartbeat worker to finish,
/// either due to shutdown signal or error.
#[derive(Debug)]
...
///
/// The worker automatically reconnects on connection failures using exponential
/// backoff with jitter. The backoff parameters are configurable via [`HeartbeatConfig`].
///
/// # Metrics
///
/// The worker emits the following metrics:
/// - `etl_heartbeat_emissions_total`: Counter of successful heartbeat emissions
/// - `etl_heartbeat_failures_total`: Counter of failed heartbeat attempts
/// - `etl_heartbeat_consecutive_failures`: Gauge of consecutive failures (resets on success)
/// - `etl_heartbeat_connection_attempts_total`: Counter of connection attempts
/// - `etl_heartbeat_last_emission_timestamp`: Gauge with Unix timestamp of last successful emission
