...
/// Smoke test: a destination that defers confirmation via the flush result callback
/// doesn't break normal pipeline operation (table copy + streaming).
#[tokio::test(flavor = "multi_thread")]
async fn destination_controlled_flush_basic_streaming() {
    init_test_tracing();
    ...
    // Insert a row and wait for it to arrive at the destination.
    ...
    // Confirm all received commits so the pipeline can advance flush LSN.
    delayed_dest.confirm_all().await;

    pipeline.shutdown_and_wait().await.unwrap();

    // Verify destination received events and shutdown was called.
    let events = delayed_dest.get_events().await;
    assert!(
        events
            .iter()
            .any(|e| matches!(e, etl::types::Event::Insert(_))),
        "destination should have received at least one insert event"
    );
    assert!(delayed_dest.shutdown_called().await);

    // Verify the destination tracked commit LSNs.
    let commit_lsns = delayed_dest.get_received_commit_lsns().await;
    assert!(
        !commit_lsns.is_empty(),
        "destination should have tracked commit LSNs"
    );
}

/// Core test: when the destination doesn't confirm, the Postgres replication slot's
...
async fn destination_controlled_flush_delays_slot_advancement() {
    ...
    // Shutdown WITHOUT confirming. The slot should not advance to the commit LSN.
    pipeline.shutdown_and_wait().await.unwrap();

    // Wait for slot to become inactive after shutdown.
    database.wait_for_slot_inactive(&apply_slot_name).await;

    // Query the slot's confirmed_flush_lsn — it should NOT have advanced to
    // the commit LSN because the destination never confirmed.
    let slot_after =
    ...
    assert!(
        slot_after < max_commit_lsn,
        "slot confirmed_flush_lsn ({slot_after:?}) should not have advanced to \
         the commit LSN ({max_commit_lsn:?}) since destination never confirmed"
    );
}

/// Positive test: confirm the batch, shutdown, verify the slot advances to the commit LSN.
#[tokio::test(flavor = "multi_thread")]
async fn destination_controlled_flush_confirm_advances_slot() {
    ...
    // Wait for the slot to become inactive after shutdown.
    database.wait_for_slot_inactive(&apply_slot_name).await;
    ...
    assert!(
        lsn_after >= max_commit_lsn,
        "after confirming, the slot LSN ({lsn_after:?}) should be at least \
...
/// Throughput test: push 100 rows through the pipeline with a background auto-confirmer
/// simulating a fast async destination. Verifies all data flows through correctly and
/// the slot advances to the final commit LSN.
#[tokio::test(flavor = "multi_thread")]
async fn destination_controlled_flush_throughput() {
    init_test_tracing();
    ...
    // Start auto-confirming in the background every 50ms (simulating a fast async destination).
    let auto_confirm = delayed_dest.start_auto_confirm(Duration::from_millis(50));

    // Wait for all insert events to arrive.
    let events_notify = delayed_dest
        .wait_for_events_count(vec![(EventType::Insert, ROW_COUNT as u64)])
        .await;

    // Insert rows and measure wall-clock time.
    let users_table_name = database_schema.users_schema().name.clone();
    let insert_start = Instant::now();
    insert_users_data(&mut database, &users_table_name, 1..=ROW_COUNT).await;
    ...
    // Final confirm to catch any remaining pending.
    delayed_dest.confirm_all().await;
    ...
    assert_eq!(
        ...
        "expected {ROW_COUNT} insert events, got {insert_count}"
    );

    // Verify all commits were tracked.
    let commit_lsns = delayed_dest.get_received_commit_lsns().await;
    assert!(
        !commit_lsns.is_empty(),
        "should have tracked commit LSNs"
    );

    // Verify slot advanced to the final commit LSN.
    let max_commit_lsn = commit_lsns.iter().copied().max().unwrap();
    let slot_lsn =
        get_confirmed_flush_lsn(database.client.as_ref().unwrap(), &apply_slot_name)
            .await
    ...
    assert!(
        ...
        "slot LSN ({slot_lsn:?}) should be at least the max commit LSN ({max_commit_lsn:?})"
    );

    // Report throughput.
    let rows_per_sec = ROW_COUNT as f64 / total_duration.as_secs_f64();
    eprintln!(
        "throughput test: {ROW_COUNT} rows in {:.2}s ({:.0} rows/sec), \
         insert time: {:.2}s, pipeline processing: {:.2}s",
        total_duration.as_secs_f64(),
        rows_per_sec,
        insert_duration.as_secs_f64(),
        (total_duration - insert_duration).as_secs_f64(),
    );
}
