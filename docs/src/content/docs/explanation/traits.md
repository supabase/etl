...
| `startup()` | Called after store caches are loaded and before workers start. Default is a no-op. Override to reconcile destination state after restarts |
...
- `startup()` runs after ETL has loaded destination metadata and table schemas from the store, so destinations can compare persisted ETL state with their physical objects before replication work starts.
