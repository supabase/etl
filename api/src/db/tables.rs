...
use sqlx::{Connection, Executor, PgConnection, PgPool, Row, postgres::PgConnectOptions};
use thiserror::Error;
...
pub async fn get_tables_with_pool(pool: &PgPool) -> Result<Vec<Table>, TablesDbError> {
    let query = r#"
        select
           	n.nspname as schema,
           	c.relname as name
