use etl_api::validation::{
    FailureType, ValidationContext, recommend_slot_wal_keep_size, validate_pipeline,
    validate_source,
};
...
#[tokio::test]
async fn slot_wal_keep_size_query_aggregates_partitions_and_applies_copy_selection() {
    let (_ctx, pool, config) = create_validation_context_with_source().await;

    for statement in [
        "create table recommendation_root (id bigint, category integer, payload text) partition \
         by range (id)",
        "create table recommendation_low partition of recommendation_root for values from (0) to \
         (10000) partition by list (category)",
        "create table recommendation_low_zero partition of recommendation_low for values in (0)",
        "create table recommendation_low_one partition of recommendation_low for values in (1)",
        "create table recommendation_low_default partition of recommendation_low default",
        "create table recommendation_high partition of recommendation_root for values from \
         (10000) to (20000) partition by hash (category)",
        "create table recommendation_high_zero partition of recommendation_high for values with \
         (modulus 2, remainder 0)",
        "create table recommendation_high_one partition of recommendation_high for values with \
         (modulus 2, remainder 1)",
        "create table recommendation_default partition of recommendation_root default",
        "create index on recommendation_root (id)",
        "insert into recommendation_root select id, id % 3, repeat(md5(id::text), 8) from \
         generate_series(0, 20499) id",
        "create table recommendation_small (id bigint primary key)",
        "insert into recommendation_small select id from generate_series(1, 10) id",
        "create publication recommendation_root_pub for table recommendation_root, \
         recommendation_small with (publish_via_partition_root = true)",
        "create publication recommendation_leaf_pub for table recommendation_root, \
         recommendation_small with (publish_via_partition_root = false)",
    ] {
        pool.execute(statement).await.unwrap();
    }

    let root_oid = sqlx::query_scalar::<_, Oid>("select 'recommendation_root'::regclass::oid")
        .fetch_one(&pool)
        .await
        .unwrap();
    let (expected_root_bytes, largest_leaf_name, largest_leaf_bytes) =
        sqlx::query_as::<_, (i64, String, i64)>(
            r#"
            with recursive relation_tree(oid, relkind) as (
                select oid, relkind
                from pg_class
                where oid = 'recommendation_root'::regclass
            ...
            leaves as (
                select
                    ...
                    pg_total_relation_size(tree.oid) as table_bytes
                from relation_tree tree
                join pg_class class on class.oid = tree.oid
                join pg_namespace namespace on namespace.oid = class.relnamespace
                where tree.relkind = 'r'
            )
            select
                (select sum(table_bytes)::bigint from leaves),
                (select table_name from leaves order by table_bytes desc, oid limit 1),
                (select table_bytes from leaves order by table_bytes desc, oid limit 1)
            "#,
        )
        .fetch_one(&pool)
        .await
        .unwrap();
    let expected_root_bytes = u64::try_from(expected_root_bytes).unwrap();
    let largest_leaf_bytes = u64::try_from(largest_leaf_bytes).unwrap();

    let all_root_tables = recommend_slot_wal_keep_size(
        &pool,
        "recommendation_root_pub",
        &TableSyncCopyConfig::IncludeAllTables,
    )
    .await
    .unwrap();
    let included_root = recommend_slot_wal_keep_size(
        &pool,
        "recommendation_root_pub",
        &TableSyncCopyConfig::IncludeTables { table_ids: vec![root_oid.0] },
    )
    .await
    .unwrap();
    let skipped_root = recommend_slot_wal_keep_size(
        &pool,
        "recommendation_root_pub",
        &TableSyncCopyConfig::SkipTables { table_ids: vec![root_oid.0] },
    )
    .await
    .unwrap();
    let all_leaf_tables = recommend_slot_wal_keep_size(
        &pool,
        "recommendation_leaf_pub",
        &TableSyncCopyConfig::IncludeAllTables,
    )
    .await
    .unwrap();
    assert_eq!(all_root_tables.largest_table_name(), "public.recommendation_root");
    assert_eq!(all_root_tables.largest_table_bytes(), expected_root_bytes);
    assert_eq!(included_root.largest_table_name(), "public.recommendation_root");
    assert_eq!(included_root.largest_table_bytes(), expected_root_bytes);
    assert_eq!(skipped_root.largest_table_name(), "public.recommendation_small");
    assert!(skipped_root.largest_table_bytes() < expected_root_bytes);
    assert_eq!(all_leaf_tables.largest_table_name(), largest_leaf_name);
    assert_eq!(all_leaf_tables.largest_table_bytes(), largest_leaf_bytes);
    assert!(
        recommend_slot_wal_keep_size(
            &pool,
            "recommendation_root_pub",
