...
//!
//! The default client, [`http::HttpK8sClient`], is backed by the [`kube`] crate
//! and talks to the cluster using the ambient configuration (in-cluster or
//! local `~/.kube/config`). Keeping the abstraction in `base` lets us swap
//! implementations in tests and non-Kubernetes environments.
//!
