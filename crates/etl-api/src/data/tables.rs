...
/// and `name` for display. For selective table-copy configuration, use the IDs
/// returned by the publication response: the generic source-tables response
/// contains both partition roots and leaves, while the publication response
/// follows its `publish_via_partition_root` setting.
