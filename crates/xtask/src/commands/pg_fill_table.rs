...
pub(crate) struct PgFillTableArgs {
    ...
    stall_seconds: u64,
    /// Use a logged table instead of the faster default UNLOGGED table.
...
impl PgFillTableArgs {
    pub(crate) fn run(self) -> Result<()> {
        ...
        print_configuration(&[
            ...
            ("Table persistence", if self.logged { "logged" } else { "unlogged" }.to_owned()),
    ...
    fn prepare_table(&self, table: &QualifiedTable) -> Result<()> {
        let persistence = if self.logged { "" } else { "unlogged " };
        ...
        let sql = format!(
            "{drop_sql}create schema if not exists {}; create {persistence}table if not exists {} \
             (worker_id integer not null, row_id bigint not null, chunk_id bigint not null, \
             payload text not null, inserted_at timestamptz not null default now()) with \
             (fillfactor = 100, autovacuum_enabled = false); alter table {} alter column payload \
