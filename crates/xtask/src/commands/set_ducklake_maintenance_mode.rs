...
fn parse_maintenance_mode(s: &str) -> Result<DuckLakeMaintenanceMode> {
    match s {
        ...
        other => bail!(
            "Unknown maintenance mode {other:?}; expected one of: disabled, kubernetes, postgres"
        ),
    }
}
...
async fn set_ducklake_maintenance_mode(
    ...
    let record = sqlx::query(
        "SELECT d.id, d.config FROM app.destinations d JOIN app.pipelines p ON p.destination_id = \
         d.id WHERE p.id = $1",
    )
    .bind(pipeline_id)
    .fetch_optional(pool)
    ...
    if dry_run {
        println!(
            "\n[set-ducklake-maintenance-mode] dry run completed successfully; no database rows \
             were updated."
        );
        return Ok(());
    }
    ...
    println!(
        "\n[set-ducklake-maintenance-mode] successfully set maintenance_mode to {mode:?} on \
         destination {destination_id} (pipeline {pipeline_id})"
    );
