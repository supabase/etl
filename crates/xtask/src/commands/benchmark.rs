...
const DEFAULT_SYNTHETIC_ROWS: u64 = 100_000;
const DEFAULT_SYNTHETIC_STREAMING_EVENTS: u64 = 100_000;
const DEFAULT_SYNTHETIC_STREAMING_BATCH_ROWS: u64 = 1_000;
...
pub(crate) struct BenchmarkArgs {
    ...
    /// Source workload to benchmark.
    #[arg(long, value_enum, default_value = "tpcc")]
    workload: Workload,
    /// Synthetic table name used by --workload synthetic.
    #[arg(long, default_value = DEFAULT_SYNTHETIC_TABLE)]
    synthetic_table: String,
    /// Synthetic rows generated by --workload synthetic.
    #[arg(long, default_value_t = DEFAULT_SYNTHETIC_ROWS)]
    synthetic_rows: u64,
    /// Synthetic CDC row updates generated by --workload synthetic.
    #[arg(long, default_value_t = DEFAULT_SYNTHETIC_STREAMING_EVENTS)]
    synthetic_streaming_events: u64,
    /// Rows updated per synthetic CDC producer statement.
    #[arg(long, default_value_t = DEFAULT_SYNTHETIC_STREAMING_BATCH_ROWS)]
    synthetic_streaming_batch_rows: u64,
...
impl BenchmarkArgs {
    pub(crate) fn run(self) -> Result<()> {
        ...
        print_configuration(&[
            ...
            ("Workload", self.workload.as_arg().to_owned()),
            ("Warehouses", self.warehouses.to_string()),
            ("TPC-C threads", tpcc_threads.to_string()),
            ("Synthetic table", self.synthetic_table.clone()),
            ("Synthetic rows", format_integer(self.synthetic_rows)),
            ("Synthetic streaming events", format_integer(self.synthetic_streaming_events)),
            ("Synthetic streaming batch rows", format_integer(self.synthetic_streaming_batch_rows)),
            ("Streaming workload", self.workload.as_arg().to_owned()),
        ...
        } else if self.needs_synthetic_table() && !self.skip_prepare {
            print_phase("Synthetic data", "preparing or reusing table");
            self.prepare_synthetic_table()?;
            print_done("Synthetic data", "ready");
        } else if self.needs_synthetic_table() {
            print_phase("Synthetic data", "checking existing table");
            self.ensure_synthetic_table_exists()?;
            print_done("Synthetic data", "existing table is ready");
        } else {
        ...
        if !self.skip_table_copy {
            ...
            let (selected_tables, table_ids, expected_row_count) = self.table_copy_inputs()?;
            print_phase(
                "Table copy",
                &format!(
    ...
    fn validate(&self) -> Result<()> {
        ...
        if self.synthetic_rows == 0 {
            bail!("--synthetic-rows must be greater than 0");
        }

        if self.synthetic_streaming_events == 0 {
            bail!("--synthetic-streaming-events must be greater than 0");
        }

        if self.synthetic_streaming_batch_rows == 0 {
            bail!("--synthetic-streaming-batch-rows must be greater than 0");
        }

        if self.synthetic_table.trim().is_empty() {
    ...
    fn table_copy_inputs(&self) -> Result<(Vec<String>, String, u64)> {
        let selected_tables = match self.workload {
            Workload::Tpcc => self.validated_tpcc_tables()?,
            Workload::Synthetic => vec![self.synthetic_table.clone()],
        };
        let table_ids = self.fetch_table_ids(&selected_tables)?;
        let expected_row_count = self.fetch_expected_row_count(&selected_tables)?;

        Ok((selected_tables, table_ids, expected_row_count))
    }

    fn prepare_synthetic_table(&self) -> Result<()> {
        if self.force_prepare {
            self.drop_synthetic_table()?;
        }

        if self.synthetic_table_ready()? {
            print_skip("Synthetic data", "table already exists with expected row count");
            return Ok(());
        }

        self.drop_synthetic_table()?;
        let table = quote_identifier(&self.synthetic_table)?;
        let rows = self.synthetic_rows;
        self.psql_status(
            &self.database,
            &format!(
                "create table {table} (
                ...
                select
                    ...
                    ((gs % 100000)::numeric / 100)::numeric(18,2),
                    (gs % 500)::integer,
                    gs % 2 = 0,
                    timestamptz '2024-01-01 00:00:00+00'
                        + ((gs % 86400)::text || ' seconds')::interval,
                    jsonb_build_object('id', gs, 'bucket', gs % 128),
                    repeat(md5(gs::text), 2)
                from generate_series(1, {rows}) as source(gs);
                analyze {table};"
            ),
        )
        .context("failed to prepare synthetic benchmark table")
    }

    fn drop_synthetic_table(&self) -> Result<()> {
        let table = quote_identifier(&self.synthetic_table)?;
        self.psql_status(&self.database, &format!("drop table if exists {table} cascade"))
            .context("failed to drop existing synthetic benchmark table")
    }

    fn synthetic_table_ready(&self) -> Result<bool> {
        if !self.public_table_exists(&self.synthetic_table)? {
            return Ok(false);
        }
    ...
    fn ensure_synthetic_table_exists(&self) -> Result<()> {
        ...
        bail!(
            "synthetic benchmark table '{}' is missing or has a different row count; rerun \
             without --skip-prepare or pass --force-prepare",
            self.synthetic_table
        )
    }

    fn public_table_exists(&self, table: &str) -> Result<bool> {
        let count = self.psql_query(
            &self.database,
            &format!(
    ...
    fn fetch_table_ids(&self, tables: &[String]) -> Result<String> {
        ...
        if table_ids.is_empty() {
            bail!("could not find selected table IDs");
        }
    ...
    fn run_table_streaming_samples(
        ...
        let selected_tables = self.streaming_tables()?;
        let mut measured_reports = Vec::with_capacity(usize::from(self.samples));
        let total_samples = self.total_sample_count();
        for sample_idx in 0..total_samples {
    ...
    fn run_table_streaming(
        ...
        let table_ids = self.fetch_table_ids(selected_tables)?;
        self.create_publication(publication_name, selected_tables)?;
        let duration_seconds = self.streaming_duration_seconds();
        let detail = match self.workload {
            Workload::Tpcc => {
            ...
            Workload::Synthetic => {
                args.extend([
                    "--synthetic-table".to_owned(),
                    self.synthetic_table.clone(),
                    "--synthetic-events".to_owned(),
                    self.synthetic_streaming_events.to_string(),
                    "--synthetic-batch-rows".to_owned(),
                    self.synthetic_streaming_batch_rows.to_string(),
                ]);
                format!(
                    "generating {} synthetic CDC updates across {} table",
                    format_integer(self.synthetic_streaming_events),
                    selected_tables.len()
                )
            }
        };
        print_phase("Table streaming", &detail);
        args.extend(["--table-ids".to_owned(), table_ids]);
    ...
    fn runs_tpcc_streaming(&self) -> bool {
        !self.skip_table_streaming && matches!(self.workload, Workload::Tpcc)
    }

    fn needs_tpcc_tables(&self) -> bool {
        matches!(self.workload, Workload::Tpcc)
            && (!self.skip_table_copy || self.runs_tpcc_streaming())
    }

    fn needs_synthetic_table(&self) -> bool {
        matches!(self.workload, Workload::Synthetic)
            && (!self.skip_table_copy || !self.skip_table_streaming)
    }

    fn streaming_tables(&self) -> Result<Vec<String>> {
        match self.workload {
            Workload::Tpcc => self.validated_tpcc_tables(),
            Workload::Synthetic => Ok(vec![self.synthetic_table.clone()]),
        }
    }

    fn streaming_duration_seconds(&self) -> u64 {
...
mod tests {
    ...
    fn aggregate_reports_uses_medians_and_records_spread() {
        ...
        assert_eq!(
            aggregate["sample_summary"]["end_to_end_with_shutdown_events_per_second"]["mean"],
            100
        );
        assert_eq!(aggregate["sample_summary"]["total_ms"]["mean"], 1033.3333333333333);
        assert_eq!(
