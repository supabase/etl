...
async fn remove_hostaddr_from_source(
    ...
    let record = match record {
        Some(r) => r,
        None => {
            bail!("Source with id {} not found", source_id);
        }
    };
    ...
    let encrypted_config = encrypt_and_serialize::<StoredSourceConfig, EncryptedStoredSourceConfig>(
        updated_config,
        encryption_keyring,
    )
    ...
    println!(
        "\n[remove-hostaddr] successfully removed hostaddr from source {}",
        source_id
    );
