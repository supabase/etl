...
pub(crate) const ETL_BATCH_SIZE_LIMIT_BYTES: &str = "etl_batch_size_limit_bytes";
...
pub(crate) fn register_metrics() {
    REGISTER_METRICS.call_once(|| {
        ...
        describe_gauge!(
            ...
            Unit::Bytes,
            "Effective memory usage in bytes, labeled by the selected system or cgroup source."
        );

        describe_gauge!(
            ETL_MEMORY_TOTAL_BYTES,
            Unit::Bytes,
            "Effective memory capacity in bytes, labeled by the selected system or cgroup source."
        );

        describe_gauge!(
            ETL_BATCH_SIZE_LIMIT_BYTES,
            Unit::Bytes,
            "Current decoded batch memory limit in bytes."
        );
