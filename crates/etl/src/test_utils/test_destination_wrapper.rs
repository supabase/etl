...
use crate::{
    ...
    destination::{
        Destination,
        async_result::{
            ApplyLoopAsyncResultMetadata, DispatchMetrics, TruncateTableResult,
                    ...
                    && next_group.row_image == RowImage::New
                {
                    ...
                    for row_index in 0..rows.len() {
                        ...
                        let old_table_row = if key_only {
                            OldTableRow::Key(rows[row_index].clone())
                        } else {
                            OldTableRow::Full(rows[row_index].clone())
                        };
                        events.push(Event::Update(UpdateEvent {
                            ...
                            tx_ordinal,
                            replicated_table_schema: replicated_table_schema.clone(),
                            updated_table_row: UpdatedTableRow::Full(new_rows[row_index].clone()),
    ...
    D: Destination + Send + Sync + Clone + 'static,
{
    ...
    async fn truncate_table(
        ...
        destination.truncate_table(table_id, wrapped_truncate_result).await?;
