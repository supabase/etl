...
mod tests {
    ...
    async fn pending_async_result_completes_before_shutdown() {
        let (_shutdown_tx, mut shutdown) = create_shutdown_channel();
        let (result_tx, pending_result) = WriteTableRowsResult::<u64>::new(());
        result_tx.send(Ok(7));

        let ShutdownResult::Ok(completed) = pending_result.with_shutdown(&mut shutdown).await
        else {
    ...
    async fn pending_async_result_stops_waiting_on_shutdown() {
        let (shutdown_tx, mut shutdown) = create_shutdown_channel();
        let (_result_tx, pending_result) = WriteTableRowsResult::<u64>::new(());
        shutdown_tx.shutdown().unwrap();

        let result = pending_result.with_shutdown(&mut shutdown).await;
