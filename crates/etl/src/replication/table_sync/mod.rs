    ...
    D: PipelineDestination,
{
    ...
    let state_type = {
        ...
        // In case the work for this table has been already done, we don't want
        // to continue and we successfully return.
        if matches!(
            state_type,
    ...
    //   perform it.
    // - `DataSync` -> a previous copy did not complete, so we restart the copy
    ...
    //   decoding state, including identity masks, and making handoff safe when
    //   no further `RELATION` message arrives.
    //
    ...
    let (start_lsn, replicated_table_schema) = match state_type {
        TableStateType::Init | TableStateType::DataSync | TableStateType::FinishedCopy => {
            ...
            //    the destination during the initial copy.
            // 2. Before the table's state is set to `FinishedCopy`, the process
            //    crashes.
            // 3. While down, the source deletes row id = 1 and inserts row id =
            //    2.
            // 4. When restarted, the process sees the table in the `DataSync`
            //    state, deletes the slot, and copies again.
            // 5. This time, only row id = 2 is copied, but row id = 1 still
            ...
            // - State store -> we write here because the table schema is used
            //   across table copying and cdc
            //  for correct decoding, thus we rely on our own state store to
            // preserve this information.
            // - Destination -> we write here because some consumers might want
            //   to have the schema of incoming
            //  data.
