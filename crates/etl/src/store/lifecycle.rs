...
//!
//! Lifecycle operations are table-scoped mutations that must keep table state,
//! versioned schemas, destination metadata, and in-memory caches consistent.
...
///
/// Provides atomic table-scoped primitives that affect both table state,
...
pub trait TableLifecycleStore {
    ...
    ///
    /// Removes table state (including history), table schemas, and
