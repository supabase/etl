...
use etl::{
    ...
    table_rows_to_arrow_batch,
    types::{
        Cell, ChangeArrowBatch, ChangeKind, ColumnSchema, DeleteEvent, Event, InsertEvent,
        OldTableRow, PgLsn, ReplicatedTableSchema, RowImage, StreamBatch, TableChangeSet, TableId,
        TableName, TableRow, TableSchema, TruncateEvent, Type as PgType, UpdateEvent,
        UpdatedTableRow,
    },
};
...
fn replicated_schema(schema: &TableSchema) -> ReplicatedTableSchema {
    ReplicatedTableSchema::all(Arc::new(schema.clone()))
}

fn insert_event(schema: &TableSchema, lsn: PgLsn, tx_ordinal: u64, table_row: TableRow) -> Event {
    Event::Insert(InsertEvent {
        start_lsn: lsn,
        commit_lsn: lsn,
        tx_ordinal,
        replicated_table_schema: replicated_schema(schema),
        table_row,
    })
}
...
fn truncate_event(schema: &TableSchema, lsn: PgLsn, tx_ordinal: u64, options: i8) -> Event {
    Event::Truncate(TruncateEvent {
        start_lsn: lsn,
        commit_lsn: lsn,
        tx_ordinal,
        options,
        truncated_tables: vec![replicated_schema(schema)],
    })
}
...
async fn write_snapshot_batch_via_destination(
    ...
    pending_result.await.into_result().expect("snapshot batch completion failed");
}
...
async fn write_stream_batches_via_destination(
    ...
    let (async_result, pending_result) =
        WriteStreamBatchesResult::new(ApplyLoopAsyncResultMetadata {
            commit_end_lsn: None,
            metrics: DispatchMetrics { items_count: batches.len(), dispatched_at: Instant::now() },
        });

    Destination::write_stream_batches(destination, batches, async_result)
        .await
        .expect("stream batch dispatch failed");
    pending_result.await.into_result().expect("stream batch completion failed");
}
...
/// `write_snapshot_batch` inserts rows through the Arrow-native destination
/// API.
#[tokio::test(flavor = "multi_thread")]
...
async fn test_write_table_rows_empty_creates_table() {
    ...
    let destination =
        DuckLakeDestination::new(catalog_url.clone(), data_url.clone(), 1, None, None, store)
            .await
            .unwrap();
    destination.write_table_rows(table_id, vec![]).await.expect("empty write failed");
...
async fn test_truncate_clears_copy_markers_for_recopy() {
    ...
    destination.write_table_rows(table_id, rows.clone()).await.unwrap();
    destination.truncate_table(table_id).await.unwrap();
    ...
    destination
        .write_events(vec![
            ...
            update_event(
                ...
                lsn,
                1,
                TableRow::new(vec![Cell::I32(1), Cell::String("Gadget".to_string())]),
                None,
            ),
            insert_event(
                &schema,
...
/// The trait-based streaming entrypoint should hand DuckLake writes off before
/// they finish.
#[cfg(feature = "test-utils")]
#[tokio::test(flavor = "multi_thread")]
async fn test_trait_write_events_dispatches_asynchronously_for_ducklake() {
    ...
    let metadata = ApplyLoopAsyncResultMetadata {
        commit_end_lsn: None,
        metrics: DispatchMetrics { items_count: 1, dispatched_at: std::time::Instant::now() },
    };
    let (async_result, pending_result) = WriteStreamBatchesResult::new(metadata);
    tokio::pin!(pending_result);

    let lsn = PgLsn::from(900_u64);
    let rows = vec![TableRow::new(vec![Cell::I32(1), Cell::String("accepted".to_string())])];
    let batch =
    ...
    assert!(
        tokio::time::timeout(Duration::from_millis(50), pending_result.as_mut()).await.is_err(),
        "async result should remain pending while the background write is paused"
    );

    release_paused_streaming_write_for_tests();

    pending_result.await.into_result().expect("background ducklake write should succeed");
...
async fn test_write_stream_batches_arrow_cdc() {
    ...
    let snapshot_rows = vec![TableRow::new(vec![Cell::I32(1), Cell::String("Widget".to_string())])];
    let snapshot_batch =
    ...
    let delete_rows = vec![TableRow::new(vec![Cell::I32(1), Cell::String("Widget".to_string())])];
    let insert_rows = vec![TableRow::new(vec![Cell::I32(2), Cell::String("Gadget".to_string())])];
...
async fn test_write_events_small_batch_stays_inlined_after_return() {
    ...
    destination
        .write_events(vec![insert_event(
            &schema,
            lsn,
            0,
            TableRow::new(vec![Cell::I32(1), Cell::String("created".to_string())]),
        )])
        .await
        .unwrap();
...
async fn test_write_events_replay_is_idempotent() {
    ...
    let batch = vec![
        ...
        update_event(
            ...
            lsn,
            1,
            TableRow::new(vec![Cell::I32(1), Cell::String("paid".to_string())]),
            Some(OldTableRow::Full(TableRow::new(vec![
                Cell::I32(1),
...
async fn test_write_events_same_commit_lsn_higher_tx_ordinal_still_applies() {
    ...
    destination
        ...
        .unwrap();
    destination
        .write_events(vec![update_event(
            &schema,
            lsn,
            1,
            TableRow::new(vec![Cell::I32(1), Cell::String("posted".to_string())]),
            Some(OldTableRow::Full(TableRow::new(vec![
                Cell::I32(1),
...
/// Restart replays should drop already-applied events before rebatching larger
/// overlaps.
#[tokio::test(flavor = "multi_thread")]
    ...
    destination
        .write_events(vec![
            ...
            insert_event(
                ...
                lsn,
                2,
                TableRow::new(vec![Cell::I32(2), Cell::String("tail".to_string())]),
            ),
        ])
        .await
        .unwrap();
...
async fn test_shutdown_flushes_inlined_cdc_rows_for_all_known_tables() {
    ...
    let store = MemoryStore::new();
    store.store_table_schema(schema_a.clone()).await.unwrap();
    store.store_table_schema(schema_b.clone()).await.unwrap();
    ...
    destination
        .write_events(vec![
            ...
            update_event(
                ...
                lsn,
                2,
                TableRow::new(vec![Cell::I32(1), Cell::String("a-one-updated".to_string())]),
                Some(OldTableRow::Full(TableRow::new(vec![
                    Cell::I32(1),
            ...
            insert_event(
                ...
                lsn,
                3,
                TableRow::new(vec![Cell::I32(2), Cell::String("b-two".to_string())]),
            ),
            delete_event(
                &schema_b,
                lsn,
...
/// A post-commit retry should replay a truncate batch without resurrecting
/// pre-truncate rows.
#[cfg(feature = "test-utils")]
#[tokio::test(flavor = "multi_thread")]
async fn test_write_events_truncate_retry_after_post_commit_failure_is_idempotent() {
    ...
    destination
        .write_events(vec![
            truncate_event(&schema, lsn, 0, 0),
            insert_event(
                &schema,
                lsn,
                1,
                TableRow::new(vec![Cell::I32(3), Cell::String("after-truncate".to_string())]),
            ),
        ])
        .await
        .unwrap();
...
/// A post-commit retry should detect the streaming progress row and avoid
