...
async fn run_table_copy_test(destination_namespace: DestinationNamespace) {
    ...
    // Sort deterministically by the debug representation as a simple stable key
    // for tests.
    actual_users
...
async fn run_cdc_streaming_test(destination_namespace: DestinationNamespace) {
    ...
    // Start pipeline from scratch (no initial data). We'll stream CDC events
    // only.
    let mut pipeline = create_pipeline(
        &database.config,
        pipeline_id,
    ...
    // Expected CDC rows: 2 inserts (UPSERT), 2 updates (UPSERT), 1 delete
    // (DELETE) Note: order here is messed up due to limitations in how
    // read_all_rows can't sort, so we sort manually by id and cdc operation
    // columns
    let expected_users = vec![
    ...
    // Sort deterministically by the primary key (id) and sequence key for
    // stable assertions
...
async fn run_cdc_streaming_with_truncate_test(destination_namespace: DestinationNamespace) {
    ...
    // After truncate, pre-truncate CDC rows should be gone (tables were
    // dropped). Only post-truncate rows remain.
    let mut actual_users = read_all_rows(&client, namespace.clone(), users_table.clone()).await;
    for row in &mut actual_users {
