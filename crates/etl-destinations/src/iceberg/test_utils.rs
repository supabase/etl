...
impl LakekeeperClient {
    ...
    pub async fn drop_warehouse(&self, warehouse_id: Uuid) -> Result<(), reqwest::Error> {
        ...
        // Even if a warehouse has no namespaces, it can still return an error
        // from a delete request if the namespace was deleted very recently. So
        // we make the best effort to attempt to delete the warehouse with
        // retries but do not fail the test if it still doesn't get deleted. At
        // worst, we'll leave some warehouses around if that happens.
