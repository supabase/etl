...
impl LakekeeperClient {
    ...
    pub async fn drop_warehouse(&self, warehouse_id: Uuid) -> Result<(), reqwest::Error> {
        ...
        // from a delete request if the namespace was deleted very
        // recently. So we make the best effort to attempt to delete the
        // warehouse with retries but do not fail the test if it still
        // doesn't get deleted. At worst, we'll leave some warehouses
