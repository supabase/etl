...
mod tests {
    ...
    fn channel_status_parsers_require_correctness_fields() {
        fn assert_required_fields<T: serde::de::DeserializeOwned>(
            ...
            let parser = std::any::type_name::<T>();
            let parse_error = serde_json::from_value::<T>(fixture.clone()).err();
            assert!(parse_error.is_none(), "parser: {parser}; error: {parse_error:?}");
