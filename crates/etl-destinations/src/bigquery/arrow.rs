...
use arrow::{
    array::{Array, ArrayRef, FixedSizeBinaryArray, StringArray},
    compute::cast,
...
use etl::{
    ...
    etl_error,
    types::{ChangeArrowBatch, TableArrowBatch, TableSchema, Type, is_array_type},
};
use gcp_bigquery_client::{
    auth::Authenticator,
    error::BQError,
    google::cloud::bigquery::storage::v1::{AppendRowsResponse, RowError, append_rows_response},
};
use prost::Message;
...
fn prepare_bigquery_arrow_column(
    ...
    let (data_type, nullable, column) = match typ {
        ...
        &Type::UUID => {
            (DataType::Utf8, true, convert_uuid_array(column).map_err(arrow_error_to_etl_error)?)
        }
        &Type::BYTEA => (
            DataType::Binary,
            true,
...
fn split_batch_into_requests(
    ...
    let request = make_arrow_append_request(
        stream_name,
        trace_id,
        if include_schema { Some(schema_bytes) } else { None },
        serialized_batch,
        batch.num_rows(),
    );
    ...
    if batch.num_rows() <= 1 {
        return Err(etl_error!(
            ...
            format!(
                "a single-row arrow append request for stream {stream_name} exceeded \
                 {max_request_size_bytes} bytes"
            )
        ));
    }
...
fn normalize_append_response(response: AppendRowsResponse) -> Result<AppendRowsResponse, Status> {
    match response.response.as_ref() {
        ...
        None => Err(Status::unavailable("append response missing append_result/error outcome")),
    }
}
...
pub(super) fn classify_append_responses(
    ...
    AppendResponseDecision { should_retry: saw_retryable_error && !saw_non_retryable_error }
}
...
fn arrow_error_to_etl_error(error: impl ToString) -> EtlError {
    etl_error!(ErrorKind::ConversionError, "bigquery arrow conversion failed", error.to_string())
}
...
async fn create_grpc_client() -> Result<ArrowBigQueryWriteClient<Channel>, BQError> {
    ...
    let channel =
        Channel::from_static(BIGQUERY_STORAGE_API_URL).tls_config(tls_config)?.connect().await?;
...
mod tests {
    ...
    use arrow::{
        array::{BinaryArray, Float32Array, Int32Array, TimestampMicrosecondArray},
        ipc::reader::StreamReader,
    };
    use chrono::NaiveDate;
    use etl::{
        conversions::arrow::table_rows_to_arrow_batch,
        types::{Cell, ColumnSchema, TableId, TableName, TableRow},
    };
    ...
    fn test_prepare_snapshot_arrow_batch_converts_supported_scalars() {
        ...
        let rows = vec![TableRow::new(vec![
            ...
            Cell::F32(1.5),
            Cell::Timestamp(
                NaiveDate::from_ymd_opt(2024, 1, 2).unwrap().and_hms_micro_opt(3, 4, 5, 6).unwrap(),
            ),
        ...
        assert!(prepared.batch.column(0).as_any().downcast_ref::<Int32Array>().is_none());
        assert!(prepared.batch.column(1).as_any().downcast_ref::<Float32Array>().is_none());
        assert!(
            prepared.batch.column(2).as_any().downcast_ref::<TimestampMicrosecondArray>().is_some()
        );
        assert!(prepared.batch.column(3).as_any().downcast_ref::<BinaryArray>().is_some());
        assert!(prepared.batch.column(4).as_any().downcast_ref::<StringArray>().is_some());
    }
