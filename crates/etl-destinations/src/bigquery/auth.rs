...
use gcp_bigquery_client::{
    ...
    yup_oauth2::{
        ...
        ServiceAccountKey,
        authenticator::{ApplicationDefaultCredentialsTypes, Authenticator as YupAuthenticator},
    ...
    P: Into<PathBuf>,
{
    ...
    authenticator.token(&[BIGQUERY_SCOPE]).await.map_err(BQError::YupAuthError)?;
...
fn adc_credential_path() -> Option<PathBuf> {
    env::var("GOOGLE_APPLICATION_CREDENTIALS").ok().map(PathBuf::from).or_else(default_adc_path)
}
