...
use crate::{
    ducklake::{
        ...
        arrow::{prepare_arrow_rows, project_primary_key_batch},
        client::{
...
pub(super) const APPLIED_BATCHES_TABLE: &str = "__etl_applied_table_batches";
...
/// retries.
pub(super) const STREAMING_PROGRESS_TABLE: &str = "__etl_streaming_progress";
...
/// Label value used when a mutation is not tied to a delete origin.
...
impl TrackedTableMutation {
    ...
    pub(super) fn sequence_key(&self) -> EventSequenceKey {
...
pub(super) fn prepare_change_table_batches(
    ...
    while let Some(group) = groups.next() {
        ...
        if starts_update_pair {
            ...
            pending_groups
                .push(groups.next().expect("peeked update pair must exist when consuming groups"));
        }
    }

    push_prepared_change_batch(&mut prepared_batches, table_schema, &table_name, pending_groups)?;
...
/// Drops already-applied Arrow-native CDC rows using the persisted sequence
/// key.
pub(super) fn retain_change_set_after_sequence_key(
    ...
    while let Some(group) = groups.next() {
                            ...
                            let first_pending_row_index =
                                first_pending_row_index(&group, last_sequence_key).ok_or_else(
                                    || {
                                        etl_error!(
                                            ErrorKind::InvalidState,
                                            "DuckLake streaming progress landed inside an Arrow \
                                             update batch without a remaining row"
                                        )
                                    },
                                )?;
                            retained_groups
        ...
        match (change_group_first_sequence_key(&group), change_group_last_sequence_key(&group)) {
    ...
    Ok(TableChangeSet { table_id: change_set.table_id, groups: retained_groups })
}
...
fn first_pending_row_index(
    ...
    group.commit_lsns.values().iter().zip(group.tx_ordinals.values().iter()).position(
        |(commit_lsn, tx_ordinal)| {
...
fn change_group_first_sequence_key(group: &ChangeArrowBatch) -> Option<EventSequenceKey> {
    group.commit_lsns.values().first().zip(group.tx_ordinals.values().first()).map(
        |(commit_lsn, tx_ordinal)| EventSequenceKey::new(PgLsn::from(*commit_lsn), *tx_ordinal),
    )
}
...
fn change_group_last_sequence_key(group: &ChangeArrowBatch) -> Option<EventSequenceKey> {
    group.commit_lsns.values().last().zip(group.tx_ordinals.values().last()).map(
        |(commit_lsn, tx_ordinal)| EventSequenceKey::new(PgLsn::from(*commit_lsn), *tx_ordinal),
    )
}
    ...
    while let Some(group) = groups.get(index) {
        match (group.change, group.row_image) {
            (ChangeKind::Insert, RowImage::New) => {
                prepared_mutations
                    .push(PreparedTableMutation::Upsert(prepare_arrow_rows(&group.rows.batch)?));
                index += 1;
            }
...
fn delete_join_predicate_from_schema(table_schema: &TableSchema) -> EtlResult<String> {
    ...
    Ok(table_schema
        .column_schemas
        .iter()
        .filter(|column_schema| column_schema.primary_key())
        .map(|column_schema| {
...
fn build_change_batch_identity(
    ...
    for group in groups {
        ...
        last_commit_lsn =
            group.commit_lsns.values().last().copied().map(PgLsn::from).or(last_commit_lsn);
...
fn insert_applied_batch_marker(
    ...
    insert_applied_copy_batch_marker(conn, &batch.table_name, &batch.batch_id)
}

/// Inserts the copy-batch marker used by snapshot replay bookkeeping.
pub(super) fn insert_applied_copy_batch_marker(
    conn: &duckdb::Connection,
    table_name: &str,
    batch_id: &str,
) -> EtlResult<()> {
    let sql = format!(
        ...
        quote_literal(DuckLakeTableBatchKind::Copy.as_str()),
        optional_lsn_to_sql_literal(None),
        optional_lsn_to_sql_literal(None),
    );
    conn.execute_batch(&sql).map_err(|error| {
        etl_error!(
...
fn update_table_streaming_progress(
    ...
    write_table_streaming_progress(conn, &batch.table_name, last_sequence_key)
}

/// Writes one table's streaming replay watermark through DuckDB.
pub(super) fn write_table_streaming_progress(
    conn: &duckdb::Connection,
    table_name: &str,
    last_sequence_key: EventSequenceKey,
) -> EtlResult<()> {
...
fn duckdb_profile_latency_seconds(conn: &duckdb::Connection) -> Option<f64> {
    conn.get_profiling_info()?.metrics.get("LATENCY")?.parse::<f64>().ok()
}

/// Records one prepared DuckLake mutation duration sample from DuckDB
/// profiling.
...
fn apply_table_mutation(
    ...
    match prepared_mutation {
        ...
        PreparedTableMutation::DeleteKeys { keys, key_columns, join_predicate, origin } => {
            histogram!(
                ETL_DUCKLAKE_DELETE_PREDICATES,
                BATCH_KIND_LABEL => batch_kind,
                DELETE_ORIGIN_LABEL => *origin,
...
fn apply_copy_mutation(
    ...
    let mut appender =
        conn.appender_to_catalog_and_db(table_name, LAKE_CATALOG, "main").map_err(|error| {
            etl_error!(
                ErrorKind::DestinationQueryFailed,
...
/// Applies one primary-key staging delete batch inside an open DuckLake
/// transaction.
fn apply_delete_keys_mutation(
    ...
    let selected_columns =
        key_columns.iter().map(|column| quote_identifier(column)).collect::<Vec<_>>().join(", ");
    ...
    let load_result = match prepared_rows {
        PreparedRows::Appender(values) => (|| {
            ...
            for value_row in values {
                appender.append_row(duckdb::appender_params_from_iter(value_row)).map_err(
                    |error| {
                        etl_error!(
                            ErrorKind::DestinationQueryFailed,
...
mod tests {
    ...
    use etl::{
        ...
        types::{ColumnSchema, TableId, TableName, Type as PgType},
    };
    ...
    fn test_prepare_change_table_batches_keeps_update_pairs_together_at_cap() {
        ...
        let insert_rows = (0..(CDC_MUTATION_BATCH_SIZE - 1))
            .map(|idx| TableRow::new(vec![Cell::I32(idx as i32), Cell::String(format!("i-{idx}"))]))
            .collect::<Vec<_>>();
        ...
        match &batches[1].action {
            PreparedDuckLakeTableBatchAction::Mutation(prepared) => {
                ...
                assert!(matches!(
                    prepared[0],
                    PreparedTableMutation::DeleteKeys { origin: "update", .. }
                ));
                match &prepared[1] {
                    PreparedTableMutation::Upsert(PreparedRows::Arrow(rows)) => {
                        assert_eq!(rows.len(), 1);
