...
{
    let duckdb_batches: Vec<DuckDbRecordBatch> =
        batches.into_iter().filter(|batch| batch.num_rows() > 0).cloned().collect();
...
mod tests {
    use arrow::{
        array::{FixedSizeBinaryArray, Int32Array, ListArray, StringArray},
