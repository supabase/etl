...
use etl::{
    ...
    types::{
        Event, EventSequenceKey, OldTableRow, ReplicationMask, SizeHint, SnapshotId, StreamBatch,
        TableArrowBatch, TableChangeSet, TableId, TableName, TableRow, TableSchema,
        UpdatedTableRow,
    },
};
...
use crate::{
    ducklake::{
        ...
        maintenance::{
            ...
            TableWriteActivity, maybe_run_requested_checkpoint,
    ...
    S: StateStore + SchemaStore + Clone + Send + Sync + 'static,
{
    ...
    async fn write_snapshot_batch_inner(&self, batch: TableArrowBatch) -> EtlResult<()> {
        ...
        .await?;
        self.merge_adjacent_files_dirty.store(true, Ordering::Release);
    ...
    async fn write_stream_batches_inner(&self, batches: Vec<StreamBatch>) -> EtlResult<()> {
        ...
        while batch_iter.peek().is_some() {
            ...
            while let Some(batch) = batch_iter.peek() {
                ...
                table_id_to_changes
                    .entry(table_id)
                    .or_insert_with(|| TableChangeSet { table_id, groups: Vec::new() })
                    .groups
        ...
        while event_iter.peek().is_some() {
                ...
                match event {
                    ...
                    Event::Update(update) => {
                        ...
                        let table_row = match update.updated_table_row {
                            ...
                            UpdatedTableRow::Partial(_) => {
                                return Err(etl_error!(
                                    ...
                                    "DuckLake row event path cannot apply partial updates"
                                ));
                            }
                        };
                        ...
                        if let Some(old_table_row) = old_table_row {
                            let old_row = old_table_row_to_table_row(old_table_row);
            ...
            while let Some(Event::Truncate(_)) = event_iter.peek() {
                if let Some(Event::Truncate(truncate)) = event_iter.next() {
                    for truncated_table in truncate.truncated_tables {
                        truncate_table_ids.entry(truncated_table.id()).or_default().push(
                            TrackedTruncateEvent::new(
                                truncate.start_lsn,
                                truncate.commit_lsn,
                                truncate.tx_ordinal,
    ...
    async fn ensure_table_exists(&self, table_id: TableId) -> EtlResult<DuckLakeTableName> {
        ...
        let table_name = self.get_or_create_table_mapping(table_id, &table_schema).await?;
...
/// Recomputes maintenance stats from the Arrow-native change suffix that still
/// needs applying.
...
mod tests {
    ...
    use etl::{
        ...
        types::{ColumnSchema, Type as PgType},
    };
    use pg_escape::{quote_identifier, quote_literal};
    ...
    async fn test_query_catalog_maintenance_metrics_reads_ducklake_metadata() {
        ...
        destination
            ...
            .await
            .expect("failed to write rows");
        destination.truncate_table(schema.id).await.expect("failed to truncate table");
