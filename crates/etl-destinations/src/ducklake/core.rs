...
/// Previous tombstone prefix recognized during upgrades and cleanup.
const LEGACY_DUCKLAKE_DROPPED_COLUMN_PREFIX: &str = "__etl_ducklake_dropped_";
...
fn is_ducklake_tombstone_column_name(column_name: &str) -> bool {
    [DUCKLAKE_DROPPED_COLUMN_PREFIX, LEGACY_DUCKLAKE_DROPPED_COLUMN_PREFIX].iter().any(|prefix| {
        let Some((ordinal, hash)) = column_name.strip_prefix(prefix).and_then(|rest| {
            let (ordinal, hash) = rest.split_once('_')?;
            Some((ordinal.parse::<i32>().ok()?, hash))
        }) else {
            return false;
        };

        ordinal > 0 && hash.len() == 16 && hash.bytes().all(|byte| byte.is_ascii_hexdigit())
    })
}
...
/// Builds the tombstone name generated by ETL versions before the prefix was
/// standardized.
fn legacy_dropped_column_tombstone_name_ducklake(column: &ColumnSchema) -> String {
    format!(
        "{LEGACY_DUCKLAKE_DROPPED_COLUMN_PREFIX}{}_{:016x}",
        column.ordinal_position,
        stable_ducklake_identifier_hash(&column.name)
    )
}
...
fn ensure_schema_diff_recoverable_ducklake(
    table_name: &DuckLakeTableName,
    diff: &SchemaDiff,
) -> EtlResult<()> {
    if diff.has_rename_cycles() {
        return Err(etl_error!(
    ...
    for operation in diff.ordered_operations() {
        match operation {
                ...
                if reused_dropped_column_names.contains(column_schema.name.as_str()) =>
            {
                ...
                let legacy_tombstone_name =
                    legacy_dropped_column_tombstone_name_ducklake(column_schema);
                ...
                let tombstone_index = find_ducklake_column(&column_names, &tombstone_name)
                    .or_else(|| find_ducklake_column(&column_names, &legacy_tombstone_name));
    ...
    let mask = previous_schema
        ...
        .map(|previous_column| {
            ...
            let tombstone_exists = [
                dropped_column_tombstone_name_ducklake(previous_column),
                legacy_dropped_column_tombstone_name_ducklake(previous_column),
            ]
            .iter()
            .any(|name| ducklake_column_name_set.contains(name.as_str()));
    ...
    S: DestinationStore,
{
    ...
    async fn handle_relation_event(
        ...
        validate_ducklake_tombstone_namespace(new_replicated_table_schema)?;
    ...
    async fn ensure_table_exists(
        ...
        validate_ducklake_tombstone_namespace(replicated_table_schema)?;
        let table_id = replicated_table_schema.id();
...
mod tests {
    ...
    #[test]
    fn plan_schema_diff_applies_rename_cycle_when_physical_order_differs_from_ordinals() {
        let diff = SchemaDiff::new(
            Vec::new(),
            Vec::new(),
            vec![rename_modification("a", "b", 1), rename_modification("b", "a", 2)],
    ...
    #[test]
    fn schema_diff_recovery_rejects_rename_cycle_without_durable_marker() {
        let diff = SchemaDiff::new(
            Vec::new(),
            Vec::new(),
            vec![rename_modification("a", "b", 1), rename_modification("b", "a", 2)],
        ...
        let error =
            ensure_schema_diff_recoverable_ducklake(&ducklake_table_name(), &diff).unwrap_err();
    ...
    #[test]
    fn plan_schema_diff_recognizes_legacy_tombstone_during_rename_replay() {
        let dropped_column = ColumnSchema::new("status".to_owned(), PgType::TEXT, -1, 3, true);
        let legacy_tombstone_name = legacy_dropped_column_tombstone_name_ducklake(&dropped_column);
        let diff = SchemaDiff::new(
            Vec::new(),
            vec![dropped_column],
            vec![rename_modification("name", "status", 2)],
        ...
        let plan = plan_schema_diff_sql_ducklake(
            &ducklake_table_name(),
            vec!["id".to_owned(), legacy_tombstone_name.clone(), "status".to_owned()],
            &diff,
        )
        .expect("legacy tombstone replay should plan");

        assert!(plan.statements.is_empty());
        assert_eq!(plan.column_names, vec!["id", legacy_tombstone_name.as_str(), "status"]);
    }
    ...
    fn tombstone_columns_to_cleanup_keeps_active_prefixed_columns() {
        ...
        let column_names = vec![
            ...
            "__etl_ducklake_dropped_4_0000000000abcdef".to_owned(),
        ];
        ...
        assert_eq!(
            ...
            vec![
                ...
                "__etl_ducklake_dropped_4_0000000000abcdef"
            ]
        );
    }
