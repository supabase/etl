...
use etl::{
    ...
    types::{ChangeKind, RowImage, SizeHint, StreamBatch, TableArrowBatch, TableId},
};
use etl_config::{
    Environment,
    shared::{
    ...
    D: PipelineDestination,
{
    ...
    async fn truncate_table(
        ...
        self.inner.truncate_table(table_id, async_result).await
    }
...
impl Destination for BenchDestination {
    ...
    async fn truncate_table(
        ...
        match self {
            Self::Null(destination) => destination.truncate_table(table_id, async_result).await,
            #[cfg(feature = "bigquery")]
            Self::BigQuery(destination) => destination.truncate_table(table_id, async_result).await,
            #[cfg(feature = "snowflake")]
            Self::Snowflake(destination) => {
                destination.truncate_table(table_id, async_result).await
            }
        }
    }
